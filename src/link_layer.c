...
int llopen(LinkLayer connection) {
  ...
  switch (connectionParameters.role) {
    case LlTx:
      ...
      stats.time_send_control += TIME_DIFF(end, start);
...
int llclose(int showStatistics) {
  switch (connectionParameters.role) {
    case LlTx:
      ...
      if (writeFrameToSerialPort(UA_Tx_Response, TRUE) < 0)
        break;
  ...
  if (showStatistics) {
    if (connectionParameters.role == LlRx) {
      ...
      time_t totalTime = TIME_DIFF(end, stats.start);
      printf("Showing link-layer protocol statistics\n"
             ...
             "  - Efficiency:\n"
             "    • Reception velocity (bits/s): %ld\n"
             "    • Overall time taken: %f seconds\n",
             stats.bytes_read,
             stats.n_frames,
             stats.n_errors,
